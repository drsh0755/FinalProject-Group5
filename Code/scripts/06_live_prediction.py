#!/usr/bin/env python3
"""
Unified Prediction Script - Handles both historical and future dates
"""
...
FEATURE_COLUMNS = [
    ...
    'Momentum', 'ROC', 'Daily_Return',
    'Volatility_10', 'Volatility_20', 'Volatility_50',
    'Volume_SMA_20', 'Volume_Ratio',
    'Price_Change', 'Price_Change_Pct', 'High_Low_Pct', 'Open_Close_Pct',
...
class UnifiedPredictor:
    def __init__(self):
        ...
        self.model = ImprovedLSTMModel(43, 64, 2, 0.3).to(self.device)
        checkpoint = torch.load(model_path, map_location=self.device)
        self.model.load_state_dict(checkpoint)
        self.model.eval()
        
        # Load historical data
        data_path = Path('Code/data/processed/spy_features_with_sentiment.csv')
    ...
    def make_prediction(self, df, current_date):
        ...
        # Prepare features
        features = df_seq[FEATURE_COLUMNS].values
            ...
            for target_date in available_dates:
                if start_date <= target_date <= end_date:
                    ...
                    if pred:
                        ...
                        print(f"✓ {target_date.strftime('%Y-%m-%d')}: ${pred['predicted_price']:.2f} ({pred['predicted_change_pct']:+.2f}%)")
    ...
    def save(self, predictions):
        ...
        output = Path('exhibition/realtime_predictions.jsonl')
