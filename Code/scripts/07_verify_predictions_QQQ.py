...
class UnifiedVerifier:
    def __init__(self):
        self.predictions_file = Path('exhibition/realtime_predictions_QQQ.jsonl')
        self.verified_file = Path('exhibition/verified_predictions_QQQ.jsonl')
    ...
    def verify_all_unverified(self):
        ...
        for i, pred in enumerate(unverified, 1):
            ...
            if verified_pred:
                ...
                print(
                    f"✓ MAPE: {verified_pred['mape']:.3f}% | Dir: {'✓' if verified_pred['direction_correct'] else '✗'}")
            else:
                skipped += 1
